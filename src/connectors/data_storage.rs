...
pub struct SqliteReader {
    ...
    /// A custom SQL query to fetch data from the table, should have the same columns as in
    /// `column_names` as well as an additional `_rowid_` column.
...
impl SqliteReader {
    ...
    fn load_table(&mut self) -> Result<(), ReadError> {
        ...
        while let Some(row) = rows.next()? {
            let (rowid, values): (i64, ValuesMap) = if self.custom_sql.is_some() {
                // For custom queries, we query by column names to allow for more flexibility
                let rowid: i64 = row.get("_rowid_")?;
                let mut values = HashMap::with_capacity(self.column_names.len());
                for column_name in self.column_names.iter() {
                    values.insert(column_name.clone(), row.get(column_name.as_str())?);
                }

                (rowid, values.into())
            } else {
                // When the query was generated by the reader itself, row columns match up with
